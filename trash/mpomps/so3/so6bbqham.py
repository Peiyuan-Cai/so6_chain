...
"""
-------------------------Spin-5/2 DMRG Hamiltonian--------------------------------
"""

#spin 5/2 site
spin5halfsite = SpinSite(S=5/2, conserve=None)

class Spin5HalfJK(CouplingModel):
    def __init__(self, model_params):
        model_params = asConfig(model_params, self.__class__.__name__)
        self.model_params = model_params
        self.Lx = model_params.get('Lx', 6)
        self.S = model_params.get('S', 5/2)
        self.pbc = model_params.get('pbc', -1)
        self.J = model_params.get('J', 1)
        self.K = model_params.get('K', 1/6)
        self.verbose = model_params.get('verbose', 2)
        ...
        site = spin5halfsite
        site.add_op('SxSx', site.multiply_operators([site.get_op('Sx'), site.get_op('Sx')]))
        site.add_op('SxSy', site.multiply_operators([site.get_op('Sx'), site.get_op('Sy')]))
        site.add_op('SxSz', site.multiply_operators([site.get_op('Sx'), site.get_op('Sz')]))
        site.add_op('SySx', site.multiply_operators([site.get_op('Sy'), site.get_op('Sx')]))
        site.add_op('SySy', site.multiply_operators([site.get_op('Sy'), site.get_op('Sy')]))
        site.add_op('SySz', site.multiply_operators([site.get_op('Sy'), site.get_op('Sz')]))
        site.add_op('SzSx', site.multiply_operators([site.get_op('Sz'), site.get_op('Sx')]))
        site.add_op('SzSy', site.multiply_operators([site.get_op('Sz'), site.get_op('Sy')]))
        site.add_op('SzSz', site.multiply_operators([site.get_op('Sz'), site.get_op('Sz')]))
    ...
    def run_dmrg(self, **kwargs):
        ...
        eng = dmrg.TwoSiteDMRGEngine(psiinit, self, dmrg_params)
        E, psidmrg = eng.run()
        print("Energy by spin 5/2 DMRG = ", E)
        self.psidmrg = psidmrg
        return psidmrg, E
...
if __name__ == "__main__":
    ...
    model_params = dict(Lx=6, J=1, K=1/6, D=300, sweeps=6)
    BBQ = BBQJK(model_params)
    BBQ.run_dmrg()
    print("---------------------------------------------------")
    model_params = dict(Lx=6, J=1, K=1/6, D=300, sweeps=6)
    spin5half = Spin5HalfJK(model_params)
    spin5half.run_dmrg()