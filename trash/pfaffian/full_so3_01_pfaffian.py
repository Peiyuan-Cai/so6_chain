"""
The Pfaffian method on SO(3) spin-1 chain with bilinear-biquadratic(BBQ) Hamiltonian ver.3 by full 1,0,-1 basis Hamiltonian

The BdG Hamiltonian is full under 1,0,-1 basis 

DMRG is used in the method by default
...
class oneparton(Site):
    ...
    if cons_N = 'Z2', it will give a Z2 symmetric physical leg
    """
...
class fullmf(bdg):
    ...
    def hamiltonian(self):
        ...
        for i in range(Nx-1):
            self.tmat[i, (i+1)%Nx] = t 
            self.dmat[i, (i+1)%Nx] = d 
        self.parity = 1
        if self.pbc:
            parity = - 1 + 2 * ( Nx % 2 )
            self.parity = parity
            self.tmat[Nx-1, 0] = t*parity 
            self.dmat[Nx-1, 0] = d*parity 
        ...
        self.tmat = self.ham[:Nc, :Nc]
        self.dmat = self.ham[:Nc, Nc:]
        #now the hamiltonian is under xyz basis
        
        #U3 = np.array([[-1j,0,1j],[1,0,1],[0,np.sqrt(2)*1j,0]])/np.sqrt(2)
        #blocks = [U3]*Nx*2
        #U = spLA.block_diag(*blocks)
        #self.ham = U @ self.ham @ U.T.conj()
...
if __name__ == "__main__":
    ...
    fullham = fullmf(chi,delta,lamb,lx,D,pbc)
    fullham.hamiltonian()
    mpsflat = []
    for i in range(1,lx+1):
        tensortemp = fullham.A_nba(i)
