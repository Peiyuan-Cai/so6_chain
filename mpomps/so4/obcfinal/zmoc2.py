...
class MPOMPS():
    ...
    def get_mpo_U1(self, v11, v22, u12, u21, qn):
        ...
        t0 = npc.zeros(legs_first, labels=['wL', 'wR', 'p', 'p*'], dtype=complex)
        i = 0
        if qn in qn_dict:
        ...
        for i in range(1,L-1):
            ti = npc.zeros(legs_bulk, labels=['wL', 'wR', 'p', 'p*'], dtype=complex)
        ...
        i = L-1
        tL = npc.zeros(legs_last, labels=['wL', 'wR', 'p', 'p*'], dtype=complex)
...
if __name__ == "__main__":
    ...
    # def apply_boundary_mode_on_gs2(name, flavor, gs):
    #     gscp = deepcopy(gs)
    #     vmat11, vmat22 = mpos.v11[:,0], mpos.v22[:,0]
    #     umat12, umat21 = mpos.u12[:,0], mpos.u21[:,0]
    #     if name == 'c':
    #         if flavor == 'w':
    #             print('getting boundary mode c_w')
    #             vmat11[:] = 0; vmat22[:] = 0; 
    #             umat21[:] = 0; umat12[:] = 0; 
    #             vmat11[0] = 1; umat21[0] = 1; vmat11[-1] = 1; umat21[-1] = -1; 
    #             print("vmat11", vmat11)
    #             print("umat21", umat21)
    #             cw = mpos.get_mpo_U1(vmat11, vmat22, umat12, umat21, 3)
    #             gscp = apply_mpo(cw, gscp)
    #         elif flavor == 'x':
    #             print('getting boundary mode c_x')
    #             vmat11[:] = 0; vmat22[:] = 0; 
    #             umat21[:] = 0; umat12[:] = 0; 
    #             vmat22[0] = 1; umat12[0] = -1; vmat22[-1] = -1; umat12[-1] = -1; 
    #             cx = mpos.get_mpo_U1(vmat11, vmat22, umat12, umat21, 1)
    #             gscp = apply_mpo(cx, gscp)
    #     elif name == 'd':
    #         if flavor == 'w':
    #             print('getting boundary mode d_w')
    #             vmat11[:] = 0; vmat22[:] = 0; 
    #             umat21[:] = 0; umat12[:] = 0; 
    #             vmat11[0] = -1; umat21[0] = -1; vmat11[-1] = -1; umat21[-1] = 1; 
    #             dw = mpos.get_mpo_U1(vmat11, vmat22, umat12, umat21, -3)
    #             gscp = apply_mpo(dw, gscp)
    #         elif flavor == 'x':
    #             print('getting boundary mode d_x')
    #             vmat11[:] = 0; vmat22[:] = 0; 
    #             umat21[:] = 0; umat12[:] = 0; 
    #             vmat22[0] = -1; umat12[0] = 1; vmat22[-1] = 1; umat12[-1] = 1; 
    #             dx = mpos.get_mpo_U1(vmat11, vmat22, umat12, umat21, -1)
    #             gscp = apply_mpo(dx, gscp)
    #     return gscp
    
    def apply_boundary_mode_on_gs(name, flavor, gs):
        gscp = deepcopy(gs)
        for l in range(gscp.L):
            gscp._B[l] *= 1+0j
        gscp.dtype = complex
        # vmat11, vmat22 = mpos.v11[:,0], mpos.v22[:,0]
        # umat12, umat21 = mpos.u12[:,0], mpos.u21[:,0]
        vmat11 = np.zeros_like(mpos.v11[:,0], dtype=complex); 
        vmat22 = np.zeros_like(mpos.v11[:,0], dtype=complex); 
        umat12 = np.zeros_like(mpos.v11[:,0], dtype=complex); 
        umat21 = np.zeros_like(mpos.v11[:,0], dtype=complex); 
        if name == 'c':
            if flavor == 'w':
                print('getting boundary mode c_w')
                # vmat11[:] = 0; vmat22[:] = 0; 
                # umat21[:] = 0; umat12[:] = 0; 
                vmat11[0] = 1j/np.sqrt(2); umat21[0] = 1j/np.sqrt(2); vmat11[-1] = 1/np.sqrt(2); umat21[-1] = -1/np.sqrt(2); 
                # print("vmat11", vmat11)
                # print("umat21", umat21)
                cw = mpos.get_mpo_U1(vmat11, vmat22, umat12, umat21, 3)
                # print("cw generated")
                ...
                # vmat11[:] = 0; vmat22[:] = 0; 
                # umat21[:] = 0; umat12[:] = 0; 
                vmat22[0] = 1/np.sqrt(2); umat12[0] = -1/np.sqrt(2); vmat22[-1] = -1j/np.sqrt(2); umat12[-1] = -1j/np.sqrt(2); 
                cx = mpos.get_mpo_U1(vmat11, vmat22, umat12, umat21, 1)
                gscp = apply_mpo(cx, gscp)
        elif name == 'd':
            if flavor == 'w':
    ...
    skip_zeromodelist = [[3,1],[3,1],[3,1],[3,1],[3,-1],[3,-1],[3,-1],[3,-1],
    ...
    def get_all_dengenracy():
        ...
        for i in range(16):
            try:
                print(' ')
                print('applying', majoprlstlst[i], 'on start state with qnlist', qnlstlst[i])
                appgs = get_start_state(skip_zeromodelist[i], initlst[i], qnlstlst[i])
                if len(majoprlstlst[i])!=0:
                    for majopr in majoprlstlst[i]:
                        appgs = apply_boundary_mode_on_gs(majopr[0], majopr[1], appgs)
                gslist.append(appgs)
