...
if __name__ == "__main__":
    ...
    v11, v22 = doublechain.wV11, doublechain.wV22
    u12, u21 = doublechain.wU12, doublechain.wU21

    print("----------MPO-MPS method: MLWO----------")
    params_mpomps = dict(cons_N=conn, cons_S=cons, trunc_params=dict(chi_max=Dmpos), pbc=pbc, init=None)
    mpos = MPOMPS(v11, v22, u12, u21, **params_mpomps)
    ...
    def obc_gs_generator(majorana_opr_list, init, qnlist):
        ...
        #     elif majopr == ['d','x']:
        #         zm_skip_list.append(-1)
        mpos.run(qnlist, init)
        appgs = mpos.psi

        try:
            #the maj application
            if len(majorana_opr_list) != 0:
                for majopr in majorana_opr_list:
    ...
    def obc_gs_generator2(majorana_opr_list, init, qnlist):
        #majoprlist = [['d','w'],['c','w']] written in the reversed order
        print(' ')
        print('OBC counting of applying MZM', majorana_opr_list)
        print('init', init)
...
'''
    ...
    for i in range(len(majoprlstlst)):
        print('majoprlstlst[i]', majoprlstlst[i])
        print('initlst[i]', initlst[i])
        print('qnlstlst[i]', qnlstlst[i])
        surgs, gpsurgs = obc_gs_generator(majoprlstlst[i], initlst[i], qnlstlst[i])
        appgs_list.append(surgs)
        if gpsurgs is not None:
            survive_appgs_list.append(gpsurgs)
