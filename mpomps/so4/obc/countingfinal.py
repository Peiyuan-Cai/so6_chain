...
if __name__ == "__main__":
    ...
    def obc_gs_generator(majorana_opr_list):
        ...
        for majopr in majorana_opr_list:
            if majopr == ['c','w']:
                ...
                zm_skip_list.append(-1)
        mpos.run_zeromode_skipper(1, zm_skip_list)
        appgs = mpos.psi
    ...
    majoprlstlst = [ [['c', 'w']], 
                    ...
                    [['d','x'], ['c','x'], ['d','w']], 
                    [['d','x'], ['c','x'], ['d','w'], ['c','w']]]
    survive_appgs_list = [gpfullgs]
    appgs_list = [no_skipped_gs]

    for majoprlst in majoprlstlst:
        surgs, gpsurgs = obc_gs_generator(majoprlst)
    ...
    '''
    for gs in appgs_list:
        norm = np.sqrt(np.real(gs.overlap(gs) * gs.overlap(gs).conj()))
        for i in range(len(gs._B)):
            gs._B[i] *= 1/norm
        print(gs.norm) #wrong normalization: we can't normalize the meaningless states?

    ovlp_mat = np.zeros((len(appgs_list), len(appgs_list)))
    for i in range(len(appgs_list)):
...
'''
    ...
    gscopy.canonical_form()
    cwgs.canonical_form()
    print("the overlap of all-skipped GS and applied GS is", gscopy.overlap(cwgs))
    
    #Gutzwiller projection
    try:
        gpcwgs = GutzwillerProjectionParton2Spin(cwgs)
        print('bbqmpo.expectation_value after application', bbqmpo.expectation_value(gpcwgs))
    ...
    #the majorana application
    operator1, operatorN, coefficient1, coefficientN = get_zeromode('d', 'w')
    dwgs = apply_zero_mode_on_gs(operator1, operatorN, coefficient1, coefficientN, skipped_gs)

    #compare with the all-skipped ground state
    gscopy.canonical_form()
    dwgs.canonical_form()
    print("the overlap of all-skipped GS and applied GS is", gscopy.overlap(dwgs))
    
    #Gutzwiller projection
    try:
        gpdwgs = GutzwillerProjectionParton2Spin(dwgs)
        print('bbqmpo.expectation_value after application', bbqmpo.expectation_value(gpdwgs))
        print('gppsimlwo.overlap(gpdwgs)', gppsimlwo_obc.overlap(gpdwgs))
        print('survived MFGS')
    except Exception as e:
        print('error:', e)

    #gs3
    mpos.run_zeromode_skipper(1, [1])
    skipped_gs = mpos.psi

    #the majorana application
    operator1, operatorN, coefficient1, coefficientN = get_zeromode('c', 'x')
    cxgs = apply_zero_mode_on_gs(operator1, operatorN, coefficient1, coefficientN, skipped_gs)

    #compare with the all-skipped ground state
    gscopy.canonical_form()
    cxgs.canonical_form()
    print("the overlap of all-skipped GS and applied GS is", gscopy.overlap(cxgs))
    
    #Gutzwiller projection
    try:
        gpcxgs = GutzwillerProjectionParton2Spin(cxgs)
        print('bbqmpo.expectation_value after application', bbqmpo.expectation_value(gpcxgs))
        print('gppsimlwo.overlap(gpcxgs)', gppsimlwo_obc.overlap(gpcxgs))
        print('survived MFGS')
    except Exception as e:
        print('error:', e)
    ...
    #the majorana application
    try:
        operator1, operatorN, coefficient1, coefficientN = get_zeromode('d', 'x')
        dxgs = apply_zero_mode_on_gs(operator1, operatorN, coefficient1, coefficientN, skipped_gs)
    except Exception as e:
        print('error:', e)
    ...
    try:
        gscopy.canonical_form()
        dxgs.canonical_form()
        print("the overlap of all-skipped GS and applied GS is", gscopy.overlap(dxgs))
    except Exception as e:
        print('error:', e)
    
    #Gutzwiller projection
    try:
        gpdxgs = GutzwillerProjectionParton2Spin(dxgs)
        print('bbqmpo.expectation_value after application', bbqmpo.expectation_value(gpdxgs))
        print('gppsimlwo.overlap(gpcxgs)', gppsimlwo_obc.overlap(gpdxgs))
        print('survived MFGS')
    except Exception as e:
        print('error:', e)
    ...
    #gs5 cwdw
    print(" ")
    mpos.run_zeromode_skipper(1, [3,-3])
    skipped_gs = mpos.psi
    ...
    try:
        ...
        dwgs = apply_zero_mode_on_gs(operator1, operatorN, coefficient1, coefficientN, skipped_gs)
        operator1, operatorN, coefficient1, coefficientN = get_zeromode('c', 'w')
        cwdwgs = apply_zero_mode_on_gs(operator1, operatorN, coefficient1, coefficientN, dwgs)
    except Exception as e:
        print('error:', e)
    ...
    try:
        gscopy.canonical_form()
        cwdwgs.canonical_form()
        print("the overlap of all-skipped GS and applied GS is", gscopy.overlap(cwdwgs))
    except Exception as e:
        print('error:', e)
    
    #Gutzwiller projection
    try:
        gpcwdwgs = GutzwillerProjectionParton2Spin(cwdwgs)
        print('bbqmpo.expectation_value after application', bbqmpo.expectation_value(gpcwdwgs))
        print('gppsimlwo.overlap(gpcxgs)', gppsimlwo_obc.overlap(gpcwdwgs))
        print('survived MFGS')
    except Exception as e:
        print('error:', e)
    ...
    #gs6 cwcx
    print(" ")
    mpos.run_zeromode_skipper(1, [3,1])
    skipped_gs = mpos.psi
    ...
    try:
        ...
        cxgs = apply_zero_mode_on_gs(operator1, operatorN, coefficient1, coefficientN, skipped_gs)
        operator1, operatorN, coefficient1, coefficientN = get_zeromode('c', 'w')
        cwcxgs = apply_zero_mode_on_gs(operator1, operatorN, coefficient1, coefficientN, cxgs)
    except Exception as e:
        print('error:', e)
    ...
    try:
        gscopy.canonical_form()
        cwcxgs.canonical_form()
        print("the overlap of all-skipped GS and applied GS is", gscopy.overlap(cwcxgs))
    except Exception as e:
        print('error:', e)
    
    #Gutzwiller projection
    try:
        gpcwcxgs = GutzwillerProjectionParton2Spin(cwcxgs)
        print('bbqmpo.expectation_value after application', bbqmpo.expectation_value(gpcwcxgs))
        print('gppsimlwo.overlap(gpcxgs)', gppsimlwo_obc.overlap(gpcwcxgs))
        print('survived MFGS')
    except Exception as e:
        print('error:', e)
'''