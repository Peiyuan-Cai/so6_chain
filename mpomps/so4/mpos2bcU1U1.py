...
Puiyuen 
    ...
    2024.10.24: Only work for cons_S='U1'
"""
...
class MPOMPS():
    ...
    def init_mps(self, init=None):
        ...
        if init is None:
            ...
            if self.pbc == 1:
                ...
                init = [5] + [10]*(L-1) #particle-hole symmetry used
                init = [10] * L
    ...
    def mpomps_step_1time(self, m, flavor):
        vm = self._V[:,m]
        um = self._U[:,m]
        v11, v22 = self.wv11[:,m], self.wv22[:,m]
        u12, u21 = self.wu12[:,m], self.wu21[:,m]
        mps = self.psi
    ...
    def run(self, init=None):
        ...
        if self.n_omode > 0:
            ...
            self.n_omode = 0
        nmode = self._U.shape[0] #the number of the modes = the real site length
