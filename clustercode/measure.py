"""
SO6 chain DMRG measure test code
"""
import numpy as np
...
if __name__ == "__main__":
    ...
    np.random.seed(0)
    np.set_printoptions(threshold=np.inf, linewidth=np.inf, precision=6, suppress=True)
    ...
    sqrd_spinspin_corr = np.zeros((36,36))
    for m in range(36):
        for n in range(36):
            sqrd_spinspin_corr += c_mn[m,n] * psi_dmrg.correlation_function('lambda'+str(m), 'lambda'+str(n))
