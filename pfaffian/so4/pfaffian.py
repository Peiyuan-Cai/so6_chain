...
def householder_complex(x):
    ...
    if sigma == 0:
        ...
        norm_x = math.sqrt(x[0].conjugate() * x[0] + sigma)
        v = np.copy(x)
...
def pfaffian_schur(A, overwrite_a=False):
    ...
    (t, z) = spLA.schur(A, output='real', overwrite_a=overwrite_a)
    l = np.diag(t, 1)
    return np.prod(l[::2]) * spLA.det(z)